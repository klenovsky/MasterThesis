...
On sample without QDs was confirmed by PL measurements 5~ML thick GaAs interlayer. Then, in PL spectrum was recognized no-phonon at $1.863$~eV and phonon at $1.834$~eV assisted indirect in $k$-space transition from $X_{xy}$ to $\Gamma$ point and their radiative time were reached. Moreover several escape energies were obtained. The most interesting results for this sample are summarized and compared with $\mathbf{k \cdot p}$~calculations in Fig.~\ref{fig:Band_scheme_wo}.
%
\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{/band_scheme/Eli_15nm_5ML_Ga05_As_0_banddiagram} %Arrhenius_PLandTRPL_all}
	\caption{Band scheme of 5~ML GaAs layer (area between vertical lines) on GaP substrate calculated by $\mathbf{k \cdot p}$ method with experimentally obtained results: Radiative transition (solid arrows) and escape processes (dotted arrows). Lengths of arrows are proportional to energies from calculations.}
	\label{fig:Band_scheme_wo}
\end{figure}


For sample with QD (without GaSb capping) three transitions at emission energies 1.750, 1.785 and 1.813~eV were detected by PL measurements. The lowest energy transition was determined as $X_{xy}$ to $\Gamma$ state in GaAs. The other two by $\mathbf{k \cdot p}$ calculations for QD In$_{0.25}$Ga$_{0.75}$As$_{0.95}$Sb$_{0.05}$ performed for electrons originated of $\Gamma$ and $L$-bands were well reproduced . The radiative times of these transitions are 20~ns for $\Gamma$ and 360~ns for $L$ electrons. Moreover escape energies from these states to GaP matrix 197~meV for $\Gamma$ and 108~meV for $L$ were found. The most interesting experimental results are compared with $\mathbf{k \cdot p}$~calculations in Fig.~\ref{fig:Band_scheme_with}.
%
\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{/band_scheme/Ga75A95}%Arrhenius_PLandTRPL_all}
	\caption{Band scheme of In$_{0.25}$Ga$_{0.75}$As$_{0.95}$Sb$_{0.05}$ QD on 5~ML/GaP calculated by $\mathbf{k \cdot p}$ method with experimentally obtained results: Radiative transition (solid arrows) and escape processes (dotted arrows). Lengths of arrows are proportional to energies from calculations.
		}
