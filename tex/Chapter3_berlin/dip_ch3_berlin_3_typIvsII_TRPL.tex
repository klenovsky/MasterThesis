...
The decrease of decay times point to indirect transition electrons from X band to heavy holes in $\Gamma$ band in GaAs, moreover weight of the processes stay same which can be connected with character of $M_1^\mathrm{w/o}$ and $M_2^\mathrm{w/o}$, where in Sec.~\ref{Sec:PL_int_wo} was discussed, that $M_1^\mathrm{w/o}$ is phonon replica of $M_2^\mathrm{w/o}$. (Co z tech dvou v TRPL je replica a co bezfonon prechod?. Je to dobre s tim typem II v rec. prostoru?)
...
On the other hand, $\tau_\mathrm{PL}$ of sample $S_\mathrm{cap}$ is slower for the high energy side of PL spectrum (energy around 1.77~eV) than for lower energy side of PL and also compared to other samples. For this reason we investigated both part individually in Figs.~\ref{fig:TRPL_int_c}--\ref{fig:TRPL_int_c_L}. 





 and the high energy side  The processes characterized by $\tau_1$ and $\tau_3$ are presented only on samples $S_\mathrm{with}$ and $S_\mathrm{cap}$, as can be seen in panels (b) in Figs.~\ref{fig:TRPL_int_w}--\ref{fig:TRPL_int_c}, stay almost constant ($\tau_3=8$~ns) or only slightly increase ($\tau_1$) with increasing excitation power which is behaviour observed for type-I nanostructures. (máš nějakou referenci, ja nemuzu nic najit)
