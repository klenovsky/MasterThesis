...
\begin{table}
	...
	\begin{tabularx}{0.9\textwidth}{cCCcc}
		...
		\midrule
		$S_\mathrm{with}$& 360.7/41 & 9.76/13& 32.20/46 &  164.0\\
		\midrule
		$S_\mathrm{cap}$ & 518.8/69 &  4.91/10& 23.11/21 & 363.7\\ 	
		\bottomrule
...
Let's start with TRPL of sample $S_\mathrm{w/o}$ depicted in Fig.~\ref{fig:TRPL_int_wo} where a decrease of decay times were observed, $\tau_1$ from 130 to 90~ns and $\tau_2$ from 30 to 13~ns. While decay times with the excitation density increase weights of the process stay the same. Because the faster process $\tau_2$ was observed across our samples and emission energy, we assume that it comes from the substrate.
...
Samples $S_\mathrm{with}$ and $S_\mathrm{cap}$ were fitted by 3ME model and have similar behaviour for $\tau_2$ and $\tau_3$ as sample $S_\mathrm{w/o}$, i.~e., a slight decrease in decay times and the same difference in weights. But, interestingly, $\tau_1$ increasing with excitation density
\begin{figure}
	\centering
