...
Looking to binding energy ($E=4-46$~meV) it seems that non-radiative losses of dynamical processes in $S_\mathrm{with}$ are caused by shallow traps. Contrary to radiative contribution in $S_\mathrm{w/o}$, we observed increasing of radiative part of both slow $\tau_1$ and as well as fast $\tau_2$ processes. The slower process starts at $\tau_\mathrm{R}^0=360$~ns and exponentially increasing proportional to $\tau_\mathrm{R}^T=26.68$~ns, whereas the faster process starts at 20.4~ns and is proportional to $\tau_\mathrm{R}^T=6.58$~ns. Energies of localized states of these two radiative times for $\tau_1$ and $\tau_2$ characterized by $T_\mathrm{C}$ are 31.7~K and 44.1~K, respectively.

In case of $S_\mathrm{cap}$
