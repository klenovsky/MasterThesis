...
\noindent Tato práce se zaměřuje na teoretické a experimentální studium kvantových teček (QDs) připravených z III--V polovodičů epitaxí z organokovových sloučenin. Pro studium byly vybrány tři systémy materiálů: (i) napětím laděné InGaAs QDs, (ii) InAs QDs překryté GaAsSb vrstvou a (iii) In$_{1-x}$Ga$_x$As$_y$Sb$_{1-y}$ QDs na GaP substrátu s 5~ML GaAs mezivrstvou. Elektronová stuktura těchto systémů byla v případě (i) počítána pomocí 8-band $\mathbf{k\cdot p}$ teorie založené na aproximaci obálkových funkcí; systémy (ii) a (iii) byly studovány pomocí fotoluminiscenční (PL) spektroskopie. 

Pro QDs vyrobené z III--V polovodičů mohou nelineární členy piezoelektrického jevu dominovat nad lineárními. Přestože energii lokalizovaných stavů započtení těchto členů ovlivňuje jen minimálně, elektrický dipól se po jejich započtení změní dramaticky a pro správný popis experimentálních dat je nutné jejich započtení. V případě napětím laděných InGaAs QDs je nutné započíst i poměrně velké vestavěné napětí vzniklé při připevňování vzorku na stolek s piezo-pohonem. Provedené výpočty ukazují, že pro naladění elektrického dipólu hrají dominantní roli výška tečky a zmíněné vestavěné napětí, což nás vedlo k vytvoření analytických modelů opírajících se o empirická pozorování pro popis vývoje dipólu v závislosti na těchto dvou parametrech.

Pomocí intenzitně a polarizačně rozlišené PL spektroskopie byla studována excitonová struktura InAs QDs, které díky tlusté krycí GaAsSb mají uvěznění typu-II s velkým modrým posuvem. Srovnáním PL spekter s vícečásticovými výpočty metodou konfigurační interakce byly identifikovány vícečásticové přechody neutrálního excitonu, biexcitonu a negativního trionu. Modrý posuv těchto vícečásticových přechodů byl srovnán s výpočty pomocí konfirační interakce se self-konzistentním cyklem.

Na mnoha místech v literatuře byly diskutovány nepřímé přechody v systémech In$_{1-x}$Ga$_x$As QDs na GaP substrátu, které se mimo jiné projevují dlouhými rekombinačními časy jichž by mohlo být s výhodou použito v zařízeních s dlouhou dobou uchování informace jako např. QD-Flash paměťové jednotky. Zkoumali jsme vliv přidání antimonu do těchto stuktur, jehož přidání u InGaAs QDs vede k typu-II uvěznění a navyšuje dobu rekombinace. 
...
\noindent This thesis deals with theoretical and experimental investigations of quantum dots (QDs) fabricated from III--V semiconductors by metalorganic vapour phase epitaxy from three material systems, (i) stress tuned InGaAs QDs, (ii) InAs QDs capped by GaAsSb layer, and (iii) In$_{1-x}$Ga$_x$As$_y$Sb$_{1-y}$ on GaP substrate with 5~ML GaAs interlayer. 
The electronic structure of the systems in case of (i) have been calculated using the envelope function approach based on 8-band $\mathbf{k\cdot p}$ theory; for systems (ii) and (iii) that was investigated by the photoluminescence (PL) spectroscopy.

The non-linear piezoelectricity in III--V semiconductor QDs dominants above the linear one. Even though them influences the energy of localized states in QD minimally, the electric dipole moment changes dramatically with respect piezoelectricity terms which leading to include second-order of piezo-electricity to reproduce experimental data. To proper modelling of the electronic structure of strain tuned InGaAs QDs have to be added high build-in in-plane prestress as an effect of bonding sample onto piezo actuator. The calculations have shown that the prestress with dot height play a dominant role in changing the dipole. Motivated by that analytical models for estimating the dipole as a function of dot height and prestress were created.

The excitonic structure of InAs QDs having due to thick GaAsSb capping layer type-II band alignment with large blue-shift was studied by intensity and polarization resolved PL. Comparing with multi-particle calculations performed by configurations interaction method in PL spectra were identified multi-particle optical transitions of neutral exciton, biexciton, and negative trion. The blue-shift of the transitions were compared with self-consistent multi-particle calculations.

%For In$_{1-x}$Ga$_x$As$_y$Sb$_{1-y}$/GaP QDs elsewhere in literature indirect optical transitions have been discussed. 
Indirect optical transitions for In$_{1-x}$Ga$_x$As/GaP QDs elsewhere in literature have been discussed. The indirect transitions resulting in long recombination time which could be used for long storage time devices as QD-Flash memories. As is well known from InGaAs QDs adding Sb into the structure leads to type-II alignment and increasing recombination time. We tested how influence that in case of In$_{1-x}$Ga$_x$As$_y$Sb$_{1-y}$/GaAs/GaP QDs.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%% INPUT ZADANI - PDF
\newpage
...
\vspace{0.2cm}
{\color{blue}{In this work, we study effects of the material composition of III--V semiconductor QDs on their electron structure. Because this thesis connects three individual topics dealing with QDs, we summarize each of them separately on the end of the appropriate chapter.
...
structure of strain-tuned InGaAs/GaAs QDs by the eight-band $\mathbf{k \cdot p}$ and the Configuration interaction method.
...
Finally, the thesis will be summarized in Chapter~\ref{chap:summary}.}}
\newpage 
