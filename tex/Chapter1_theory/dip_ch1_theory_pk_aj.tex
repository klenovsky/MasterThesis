...
&-& \sum_{j, i} \frac{Z_j e^2}{4\pi \epsilon(\mathbf{r}_i, \mathbf{R}_{j}) |\mathbf{r}_i-\mathbf{R}_{j}|} + \frac{1}{2}\sum_{i', i, i\neq i'} \frac{e^2}{4\pi \epsilon(\mathbf{r}_i, \mathbf{r}_{i'}) |\mathbf{r}_i-\mathbf{r}_{i'}|}. \label{eq:HAM_idealni_krystal}
\end{eqnarray}
In the above expression the index $i$ ($j$) denotes summing over electrons (nuclei). The symbol $\mathbf{p}_i$ ($\mathbf{P}_j$) denotes momentum operator of the electrons (nuclei), $m_i$ ($M_j$) is the mass of electron (nucleus) $\mathbf{r}_i$ ($\mathbf{R}_j$) is the position of the $i$th electron ($j$th nucleus), $Z_j$ is the atomic number of the nucleus; $e$ and $\epsilon$ represent the electronic charge and the spatially dependant permittivity, respectively.
...
Ions are much heavier than electrons, so they move significantly more slowly. % than the electrons. 
Therefore, electrons can respond to nuclei movement almost immediately or, in other words, to the electrons the ions seem as static background charges. On the other hand, nuclei cannot follow electrons' motion and they 'feel' only a time-averaged electronic potential. With the adiabatic approximation Hamiltonian~(\ref{eq:HAM_idealni_krystal}) can be simplified to
...
Diagonalizing the hamiltonian $H_\mathrm{e}$ for $N$ electrons in the system (there are $>10^{23}$ electrons/cm$^3$ in a semiconductor) is currently an unsolvable task. To simplify this problem we introduce the mean-field approximation, where we assume that each electron is influenced only by a mean electrostatic potential~$V\left(\mathbf{r}\right)$. Thus the Schrödinger equation describing the motion of each electron will be identical and given by
...
The $\mathbf{k\cdot p}$ approximation was originally developed for calculations of the band structure of bulk semiconductors by Luttinger~\citep{Lutt} and Kane~\citep{Kane}. It is based on the fact that the most crucial properties of semiconductors come from the vicinity of the extrema of the valence and the conduction band. Then, the band structure can be reconstructed from the eigenstates at those extrema.
...
\end{equation}
The solution of crystalline solid can then be searched for in accordance with the Bloch theorem as a product of a plane wave and a function $u_{n\mathbf{k}}(\mathbf{r})$ with the periodicity of the crystalline lattice
...
If we insert (\ref{eq:Bloch}) into (\ref{eq:1el}) and act with the operator in round brackets on the Bloch function. We then arrive at the Schrödinger equation for the periodic part of wavefunction with the $\mathbf{k}\cdot\mathbf{p}$ term which gives the name to the method
...
The solution of the (\ref{eq:kp}) can be expanded as a superposition of all $u$-function for any given $\mathbf{k}$ in the reciprocal space. For QDs fabricated from direct zinc-blende semiconductors, where the minimum of conductive and the maximum of valance band are situated at $\Gamma$ point of the Brillouin zone, usually $\mathbf{k}=0$ is selected and this expansion reads
...
If we insert the expansion into (\ref{eq:kp}), multiply both sides by $u_{j0}^*(\mathbf{r})$ and integrate over the unit cell, the equation reduces at $\Gamma$ point to 
\begin{equation}
...
\end{pmatrix},
\end{equation}
where $E_\mathrm{c}$ and $E_\mathrm{v}$ represent conduction and valance band energies at $\Gamma$ point, respectively, $P$ symbolizes the optical matrix element, $P=\langle s|p_x|x\rangle=\langle s|p_y|y\rangle=\langle s|p_z|z\rangle$.
...
\end{eqnarray*}
the equations for $K_y$ and $K_z$ are identical except for cyclic index commutation; $A'$ and $B$ are the Kane's parameters~\citep{Kane} and $N'$, $L'$, $M$ are the Dresselhaus parameters~\citep{Dress}.
...
& D_x& n\epsilon_{xy}& n\epsilon_{xz}& 0& 0& 0& 0\\
& & D_y& n\epsilon_{yz}& 0& 0& 0& 0\\
& & & D_z& 0& 0& 0& 0\\
& & & & D_s& ND_{sx}& ND_{sy}& ND_{sz}\\
& & & & & D_x& n\epsilon_{xy}& n\epsilon_{xz}\\
& & & & & & D_y& n\epsilon_{yz}\\
& & & & & & & D_z\\\end{pmatrix}.
\end{equation}
The symbols $\epsilon_{ij}$ ($i,j=x,y,z$) are the elements of elastic strain tensor and the other parameters are

\begin{eqnarray*}
	D_s=a_c(\epsilon_{xx}+\epsilon_{yy}+\epsilon_{zz}),\nonumber\\
	D_i=l\epsilon_{ii}+m(\epsilon_{jj}+\epsilon_{kk}),\nonumber\\
	ND_{si}=b'\epsilon_{jk}-\mathrm{i}P\sum_\alpha\epsilon_{i\alpha}k_\alpha,\nonumber\\
\end{eqnarray*}
...
The electric polarization $\mathbf{P}_{{\rm piezo}}$ can be expanded in the Taylor series of a strain tensor $\epsilon$ as
\begin{equation}
\mathbf{P}_{{\rm piezo},\mu}(\mathbf{r})=%\sum_{j} e_{\eta j}(\mathbf{r})\epsilon_{j}(\mathbf{r}),
\sum_je_{\mu j}\epsilon_j+\frac{1}{2}\sum_{jk}B_{\mu jk}\epsilon_j\epsilon_k+\dots ,\label{eq:second_order}
\end{equation}
where $e_{\mu j}$ represents linear and $B_{\mu jk}$ quadratic piezoelectric coefficients, $\epsilon$-s are indexed according to the Voigt notation (i.e. $\epsilon_1=\epsilon_{xx}$, $\epsilon_2=\epsilon_{yy}$, $\eta_3=\epsilon_{zz}$, $\epsilon_4=2\epsilon_{yz}$, $\epsilon_5=2\epsilon_{xz}$, $\epsilon_6=2\epsilon_{xy}$)~\citep{voigt_notation, Beya-Wakata2011}. 
...
e_{14 }\epsilon_4 \label{eq:first_order}.
\end{equation}
Then according to Coulomb law, the piezoelectric charge $\rho_{\rm piezo}(\bf{r})$ can be written using the divergence of displacement field $\bf{ D}$ in case if the external electric field is not affected ($\mathbf{D}=\mathbf{P}_\mathrm{piezo}$) leading to the Poisson equation
...
\end{eqnarray}
where $\epsilon_r(\bf{r})$ is the spatially dependent static dielectric function and $V_{\rm piezo}(\bf{r})$ is the piezoelectric potential generated by the Poisson equation. The most considerable effect of the piezoelectric potential to the one-particle eigenstates is their elongation in $[110]$ or $[1\overline{1}0]$ directions~\citep{Stier1999} which has consequences such as the fine structure splitting (FSS) of the bright excitonic states. FSS is an undesirable phenomenon preventing a creation of entangled photon pairs from single QD required to realize, e.~g., a single photon source. To reduce FSS, several experimental methods have been tried including InAs QDs growth on $[111]$~substrates~\citep{StockFSS}, fabrication of strain-free GaAs/AlGaAs~\citep{Abbarchi_2008} QDs with zero piezoelectric field or tuning FSS to zero by external fields: electric~\citep{Gerardot_2007, Vogel_2007}, magnetic~\citep{Stevenson_2006} or strain~\citep{kleDresden}. 
...
\end{equation}
%
By applying this approximation, we correctly describe not only the direct Coulomb interaction caused by the influence of one charge carrier on the effective Coulomb potential from the remaining particles but also via so-called exchange term $\psi_{\rm{e}}(\mathbf{r}_{\rm{h}}) \psi_{\rm{h}}(\mathbf{r}_{\rm{e}})$. This approach still does not describe the correlation effects originated from the interaction of two equally charged particles with the same spin. To include those effects, the multi-particle wavefunction can be expanded into the series of the Slater determinants which is a fundamental principle of configuration interaction (CI) method~\citep{t_stier}.
...
\end{equation}
%
where $E^M$ is the eigenenergy of the (multi-)excitonic state $\left|M\right>$ describes the multi-particle complex with $N_{\rm{e}}$ electrons and $N_{\rm{h}}$ holes. In the following, examples of wave functions for selected complexes, that are explored in this thesis, are listed. For the neutral exciton $X^0$ ($N_{\rm{e}}=1$, $N_{\rm{h}}=1$), we have
%
\begin{equation}
\label{eq:suppl:CIWavefunctionX}
\left|X^0\right>=\sum^{n_{\rm{e}}}_{i=1}\sum^{n_{\rm{h}}}_{j=1}\eta_{ij}
\begin{vmatrix}
...
\label{eq:suppl:CIWavefunctionX+}
\left|X^+\right>=
\sum^{n_{\rm{e}}}_{i=1}\,\sum^{n_{\rm{h}}}_{\substack{j,k=1\\k>j}}\eta^+_{ijk}
\begin{vmatrix}
...
\label{eq:suppl:CIWavefunctionX-}
\left|X^-\right>=
\sum^{n_{\rm{e}}}_{\substack{i,j=1\\j>i}}\,\sum^{n_{\rm{h}}}_{k=1}\eta^-_{ijk}
\begin{vmatrix}
...
\left|XX^0\right>=
\sum^{n_{\rm{e}}}_{\substack{i,j=1\\j>i}}\,\sum^{n_{\rm{h}}}_{\substack{k,l=1\\k>l}}\eta^{XX}_{ijkl}
\begin{vmatrix}
...
For more details about that method we refer to~\citep{Klenovsky2017}.
